...
<link rel="stylesheet" href="{{asset('css/deposito.css')}}">
...
<section class="container">
    ...
    <form action="{{ route('simulasi.kredit') }}" method="POST">
        @csrf
        <input type="number" name="loanAmount" placeholder="Masukkan jumlah pinjaman" value="{{ old('loanAmount') }}">

        <input type="number" name="interestRate" placeholder="Masukkan bunga per tahun"
            value="{{ old('interestRate') }}">

        <input type="number" name="loanTerm" placeholder="Masukkan jangka waktu (bulan)" value="{{ old('loanTerm') }}">

        <select name="method">
            <option value="anuitas" {{ old('method') == 'anuitas' ? 'selected' : '' }}>Anuitas</option>
            <option value="demandloan" {{ old('method') == 'demandloan' ? 'selected' : '' }}>LJT</option>
        </select>
